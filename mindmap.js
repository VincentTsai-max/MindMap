...
(function () {
    ...
    /* sideOf：取「有效顯示側」——未定側（null／undefined）一律當 R 顯示，供繪製/佈局使用 */
    function sideOf(n) { return n && n.side === 'L' ? 'L' : 'R'; }
    /* rawSide：取「實際已定側」——L / R / null（尚未指定）。統計左右數量時必須用這個，
       否則 side===null 的既有節點會被 sideOf 誤判成 R，導致新節點永遠往右塞（左邊被算成 0）。 */
    function rawSide(n) { return (n && (n.side === 'L' || n.side === 'R')) ? n.side : null; }

    /* 依目前已定側的數量挑較少的一邊（右側優先）；未定側者不計入，避免灌水 */
    function pickBalancedSide(centralId) {
        var kids = childrenOf(centralId), r = 0, l = 0;
        for (var i = 0; i < kids.length; i++) {
            var rs = rawSide(kids[i]);
            if (rs === 'L') l++; else if (rs === 'R') r++;
        }
        return (r <= l) ? 'R' : 'L';
    }
    ...
    function rebalanceSides() {
        ...
        var kids = childrenOf(state.centralId);
        if (kids.length < 2) { toast('子主題不足，不需要重新排列'); return 0; }
        pushUndo();
        var unpinned = 0;
        for (var i = 0; i < kids.length; i++) {
            kids[i].side = (i % 2 === 0) ? 'R' : 'L';
            /* 手動拖曳過的節點會被釘住位置而脫離自動排版，重新平衡時一併放回流內，
               否則只換了左右歸屬、位置仍釘在原地，看起來像沒生效 */
            if (isPinned(kids[i])) { kids[i].posX = null; kids[i].posY = null; unpinned++; }
        }
        afterMutate();
        toast('已重新平衡左右各 ' + Math.ceil(kids.length / 2) + ' / ' + Math.floor(kids.length / 2) + ' 個主題'
              + (unpinned ? '（並解除 ' + unpinned + ' 個手動位置）' : ''));
        return kids.length;
    }
    ...
    STRUCTURES['mindmap'] = {
        measure: function (n, depth, idx) {
            ...
            /* 第一層左右平衡（沿 v1：未指定側邊者補到較少的一側，右側優先）——
               用 rawSide 只統計「已明確定側」的節點，避免 null 被誤判灌到右側 */
            var rC = 0, lC = 0;
            for (i = 0; i < kids.length; i++) {
                var rsK = rawSide(kids[i]);
                if (rsK === 'L') lC++;
                else if (rsK === 'R') rC++;
            }
            for (i = 0; i < kids.length; i++) {
    ...
    /* 開新視窗看原圖：不能直接 window.open(dataURL)——Chrome 60 起禁止分頁直接載入 data: URL，
       會開出空白頁。改成開空白視窗後寫入一個 <img> 來承載。 */
    function openImageWindow(fileId) {
        var src = fileUrl(fileId);
        if (!src) { toast('圖片尚未載入完成，請稍候再試', true); return; }
        var w = window.open('', '_blank');
        if (!w) { toast('瀏覽器封鎖了彈出視窗，請允許後再試', true); return; }
        var doc = w.document;
        doc.open();
        doc.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>' +
                  ((state.title || '心智圖') + ' — 圖片').replace(/[<>&]/g, '') +
                  '</title><style>html,body{margin:0;height:100%;background:#1B1F24;' +
        ...
        if (im) im.src = src;   /* 用屬性指派，避免內嵌超長 base64 造成 document.write 過大 */
    }
    ...
    function addChild(parentId, text) {
        ...
        if (parentId === state.centralId) {
            side = pickBalancedSide(state.centralId);
        }
        var id = uid();
        state.nodes[id] = {
    ...
    function onPointerDown(e) {
        ...
        if (imgEl) {
            ...
            if (imR) { openImageWindow(imR.fileId); }
            e.preventDefault();
            return;
        }
